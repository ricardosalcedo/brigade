...
def main():
    ...
    if not run_command(
        "pip3 install -r requirements-test.txt", "Installing test dependencies"
    ):
    ...
    total_tests += 1
    if run_command(
        "black --check core/ analyzers/ workflows/", "Code formatting (black)"
    ):
        tests_passed += 1

    total_tests += 1
    if run_command(
        "isort --check-only core/ analyzers/ workflows/", "Import sorting (isort)"
    ):
        tests_passed += 1
