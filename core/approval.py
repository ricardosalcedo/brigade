...
class ApprovalManager:
    ...
    def request_pr_approval(self, 
        ...
        if fixes:
            ...
            for i, fix in enumerate(fixes[:5], 1):
                issue_desc = fix.get('issue_description', fix.get('description', 'Code improvement'))
        ...
        while True:
            try:
                response = input("\n   [y]es / [n]o / [d]etails / [s]ave for later: ").lower().strip()
                ...
                if response in ['y', 'yes']:
                    ...
                    print("   Please enter 'y' (yes), 'n' (no), 'd' (details), or 's' (save)")
                    
            except KeyboardInterrupt:
                print("\n❌ Approval cancelled")
                return False
    
    def _show_detailed_fixes(self, fixes: List[Dict[str, Any]], analysis: Dict[str, Any]):
    ...
    def _save_for_later(self, file_path: str, fixes: List[Dict[str, Any]], analysis: Dict[str, Any]):
