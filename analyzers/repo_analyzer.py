...
class RepositoryAnalyzer(BaseAnalyzer):
    ...
    def _categorize_file(self, file_path: Path) -> str:
        ...
        # Configuration files
        config_patterns = ['config', 'settings', '.env', 'requirements', 'package.json', 'Dockerfile']
    ...
    def _split_files_into_chunks(self, files: List[str], category: str, priority: int) -> List[RepoChunk]:
        ...
        for file_path in files:
            ...
            if (current_size + file_size > self.max_chunk_size or 
                len(current_chunk) >= self.max_files_per_chunk):
    ...
    def _summarize_chunk(self, file_analyses: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        avg_quality = sum(f.get('quality_score', 0) for f in file_analyses) / total_files
    ...
    def _synthesize_results(self, repo_summary: RepoSummary, chunk_results: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        repo_quality = sum(total_quality_scores) / len(total_quality_scores) if total_quality_scores else 0
    ...
    def _generate_insights(self, chunk_results: List[Dict[str, Any]], repo_quality: float, all_issues: Dict[str, int]) -> List[str]:
        """Generate AI-powered insights about the repository"""
        insights = []
        
        # Quality insights
        if repo_quality >= 8:
            insights.append("🎯 Excellent code quality - repository shows strong engineering practices")
        elif repo_quality >= 6:
            insights.append("⚡ Good code quality with room for targeted improvements")
        else:
            insights.append("🔧 Code quality needs attention - consider systematic refactoring")
        ...
        if len(test_chunks) == 0:
            ...
            if test_ratio < 0.3:
                insights.append("📊 Low test-to-code ratio - consider expanding test coverage")
        
        return insights
    
    def _generate_recommendations(self, all_issues: Dict[str, int], repo_quality: float) -> List[str]:
        ...
        if 'style' in all_issues and all_issues['style'] > 20:
            recommendations.append("3. Set up automated code formatting (black, prettier)")
        
        if 'complexity' in all_issues:
            recommendations.append("4. Refactor complex functions for better maintainability")
