...
class StaticAnalyzer(BaseAnalyzer, ICodeAnalyzer):
    ...
    def analyze_file(self, file_path: str) -> AnalysisResult:
        ...
        if not language:
            raise UnsupportedFileTypeError(
                f"Could not detect language for: {file_path}"
            )
        ...
        try:
            ...
            return AnalysisResult(
                ...
                metadata={
                    "analyzer": "static",
                    "tools_used": self._get_tools_used(language),
                },
            )
    ...
    def _analyze_python(self, file_path: str) -> List[Dict[str, Any]]:
        ...
        # Flake8 analysis
        flake8_result = ProcessUtils.run_command(
            ["flake8", "--format=json", file_path], self.timeout
        )
    ...
    def _analyze_javascript(self, file_path: str) -> List[Dict[str, Any]]:
        ...
        # ESLint analysis
        eslint_result = ProcessUtils.run_command(
            ["eslint", "--format=json", file_path], self.timeout
        )
    ...
    def _normalize_flake8_issues(
        self, flake8_issues: List[Dict]
    ) -> List[Dict[str, Any]]:
        ...
        for issue in flake8_issues:
            normalized.append(
                {
                    "type": (
                        "style" if issue.get("code", "").startswith("E") else "warning"
                    ),
                    "severity": "medium",
    ...
    def _normalize_eslint_issues(
        self, eslint_issues: List[Dict]
    ) -> List[Dict[str, Any]]:
