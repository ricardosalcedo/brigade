...
class UnifiedAnalyzer(BaseAnalyzer, ICodeAnalyzer):
    ...
    def _combine_results(
        self, static_result: AnalysisResult, llm_result: AnalysisResult
    ) -> AnalysisResult:
        ...
        combined_recommendations = list(
            set(static_result.recommendations + llm_result.recommendations)
        )
    ...
    def _is_duplicate_issue(
        self, new_issue: Dict[str, Any], existing_issues: List[Dict[str, Any]]
    ) -> bool:
        """Check if issue is duplicate"""
        for existing in existing_issues:
            if (
                existing.get("type") == new_issue.get("type")
                and existing.get("line") == new_issue.get("line")
                and existing.get("description", "").lower()
                in new_issue.get("description", "").lower()
            ):
                return True
        return False
